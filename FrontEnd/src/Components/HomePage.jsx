...
function HomePage(props) {
  ...
  <Slider {...settings}>
    {["banner_1.png", "banner_2.png", "banner_3.jpg"].map((image, index) => (
      <div key={index} className="relative w-full">
        <img src={`/src/assets/${image}`} alt="banner" className="w-full h-[50vw] sm:h-[40vw] lg:h-[39vw] object-cover rounded-lg" />
        <div className="absolute top-0 left-0 w-full h-full flex flex-col justify-center p-6 sm:p-12 bg-black/30">
          <h3 className="text-white text-3xl sm:text-5xl font-semibold tracking-widest w-80">
          Elevate Your Style
          </h3>
          <p className="text-slate-100 text-sm sm:text-lg mt-2 sm:mt-5 w-96">
          Discover the trends in men's fashion, from casual to formal wear.
          </p>
          <button className="p-2 sm:p-3 bg-green-600 mt-3 sm:mt-5 px-6 sm:px-10 rounded-lg text-white w-60">
        ...
        <div className="flex flex-col gap-5 p-5 w-full lg:w-auto  ">
          {["small_banner_1.png", "small_banner_2.png"].map((image, index) => (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 p-5 text-slate-900">
        {["small_banner_3.png", "small_banner_4.png", "small_banner_5.png"].map((image, index) => (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 p-5">
        {["small_banner_6.png", "small_banner_7.png"].map((image, index) => (
